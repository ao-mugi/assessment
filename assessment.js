...
assessmentButton.addEventListener( //イベント検知設定の追加
  'click', //クリックイベント //イベント検知をしたら実行される
  function () {
    ...
    script.setAttribute('src', 'https://platform.twitter.com/widgets.js');//cre属性を追加
 ...
 * @return {string} 診断結果
 */
function assessment(userName) { //functionは関数で　assessmentは評価
  ...
  for (let i = 0; i < userName.length; i++) { //文字回数ループ
    ...
    //charCodeAt の意味は、文字列の中のある文字を、数値（文字コード）に変換する
  }
